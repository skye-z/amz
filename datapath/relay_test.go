...
import (
	...
	"github.com/skye-z/amz/datapath"
)

// 验证数据面会暴露最小收发接口与统计信息。
func TestNewPacketIO(t *testing.T) {
	io := datapath.NewPacketIO(1600)
	if io.MTU() != 1600 {
...
func TestPacketIOForwardUplinkFragmentsAndForwardsICMP(t *testing.T) {
	...
	go func() {
		done <- io.ForwardUplink(ctx, dev, endpoint)
	}()

	waitForCondition(t, time.Second, func() bool {
		return len(endpoint.Writes()) == 3 && len(dev.Writes()) == 1
	})
	cancel()
...
func TestPacketIOForwardDownlinkWritesBackToDevice(t *testing.T) {
	...
	go func() {
		done <- io.ForwardDownlink(ctx, endpoint, dev)
	}()
...
// 验证双向 relay 在上下文取消时会关闭远端数据面，避免卡死在阻塞读取。
func TestPacketIORelayClosesEndpointOnCancel(t *testing.T) {
	...
	go func() {
		done <- io.Relay(ctx, dev, endpoint)
	}()
...
func (d *stubPacketDevice) EnqueueRead(packet []byte) {
	d.readCh <- append([]byte(nil), packet...)
}
...
func (e *stubPacketEndpoint) EnqueueRead(packet []byte) {
...
func expectPacket(t *testing.T, got, want []byte) {
	t.Helper()
	if len(got) != len(want) {
		t.Fatalf("expected packet len %d, got %d", len(want), len(got))
	}
	for i := range want {
		if got[i] != want[i] {
