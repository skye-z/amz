...
func New(cfg config.KernelConfig) (*Manager, error) {
	...
	return &Manager{cfg: clone, state: types.StateIdle, listen: clone.HTTP.ListenAddress}, nil
}
...
func (m *Manager) Snapshot() Snapshot {
	m.mu.Lock()
	defer m.mu.Unlock()
	return Snapshot{ListenAddress: m.listen, ReuseTunnelLifecycle: true}
}
...
func (m *Manager) SetHTTPDialer(dialer StreamDialer) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.dialer = dialer
	m.transport = nil
...
func (m *Manager) SetHTTPRoundTripper(roundTripper http.RoundTripper) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager) start(ctx context.Context, provided net.Listener) error {
	...
	server := &http.Server{Handler: &handler{manager: m}, BaseContext: func(net.Listener) context.Context { return context.Background() }}
	m.listener = ln
	m.server = server
...
func (h *handler) handleConnectViaStream(w http.ResponseWriter, r *http.Request, streamMgr ConnectStreamOpener) {
...
func (m *Manager) currentHTTPDialer() StreamDialer {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.dialer != nil {
		return m.dialer
...
func (m *Manager) currentStreamManager() ConnectStreamOpener {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.streamManager
}
func (m *Manager) currentRoundTripper() http.RoundTripper {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.transport != nil {
		return m.transport
