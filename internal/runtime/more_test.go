...
func TestUpstreamAndDNSDialers(t *testing.T) {
	...
	base := &recordingDialer{connFactory: func() net.Conn { client, server := net.Pipe(); go func() { defer server.Close(); req, _ := http.ReadRequest(bufio.NewReader(server)); if req != nil { _, _ = server.Write([]byte("HTTP/1.1 200 Connection Established\r\n\r\n")) } }(); return client }}
...
func (l *captureRuntimeLogger) Printf(format string, args ...any) { l.lines = append(l.lines, fmt.Sprintf(format, args...)) }
