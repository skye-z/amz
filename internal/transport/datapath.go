...
func (p *PacketIO) readUplinkPacket(ctx context.Context, dev Device) (*packet.Buffer, int, error) {
	buf := p.pool.Get()
...
func (p *PacketIO) forwardUplinkFragments(ctx context.Context, dev Device, endpoint PacketRelayEndpoint, packet []byte) error {
...
func (p *PacketIO) forwardUplinkFragment(ctx context.Context, dev Device, endpoint PacketRelayEndpoint, fragment []byte) error {
...
func (p *PacketIO) writeICMPPacket(ctx context.Context, dev Device, packet []byte) error {
	if len(packet) == 0 {
		return nil
	}
...
func (p *PacketIO) writeDownlinkPacket(ctx context.Context, dev Device, packet []byte, want int) error {
	written, err := dev.WritePacket(ctx, packet)
	if err != nil {
