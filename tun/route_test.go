...
func TestTUNDeviceInterface(t *testing.T) {
	var dev tun.TUNDevice = &stubTUNDevice{name: "igara0", mtu: 1280, read: 4, writ: 4}
...
func TestRouteModes(t *testing.T) {
	if tun.RouteModeGlobal.String() != "global" {
		t.Fatalf("expected global mode string, got %q", tun.RouteModeGlobal.String())
	}
	if tun.RouteModeSplit.String() != "split" {
		t.Fatalf("expected split mode string, got %q", tun.RouteModeSplit.String())
	}
	if tun.RouteMode("custom").Valid() {
		t.Fatal("expected custom mode to be invalid")
	}
	if !tun.RouteModeGlobal.Valid() || !tun.RouteModeSplit.Valid() {
...
func TestRoutePlanClone(t *testing.T) {
	...
	if clone.Mode != tun.RouteModeGlobal {
...
func TestRouteManagerApply(t *testing.T) {
	manager, err := tun.NewRouteManager(tun.RoutePlan{
	...
	if snapshot.Mode != tun.RouteModeSplit {
...
func TestRouteManagerRejectsInvalidInput(t *testing.T) {
	if _, err := tun.NewRouteManager(tun.RoutePlan{}); err == nil {
