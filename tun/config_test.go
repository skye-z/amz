...
func TestFakeTUNDevicePacketFlow(t *testing.T) {
	dev, err := tun.NewFakeTUNDevice(tun.TUNDeviceConfig{
		Name: "igara0",
		MTU:  1400,
	})
	if err != nil {
...
func TestTUNConfigurationValidate(t *testing.T) {
	valid := tun.TUNConfiguration{
		Device: tun.TUNDeviceConfig{
			Name: "igara0",
			MTU:  1280,
		},
		Addresses: []tun.TUNAddress{
			{CIDR: "172.16.0.2/32"},
	...
	tests := []struct {
		name string
		cfg  tun.TUNConfiguration
	}{
		{
			name: "missing device name",
			cfg: tun.TUNConfiguration{
				Device:    tun.TUNDeviceConfig{MTU: 1280},
				Addresses: []tun.TUNAddress{{CIDR: "172.16.0.2/32"}},
			},
		},
		{
			name: "mtu too small",
			cfg: tun.TUNConfiguration{
				Device:    tun.TUNDeviceConfig{Name: "igara0", MTU: 1200},
				Addresses: []tun.TUNAddress{{CIDR: "172.16.0.2/32"}},
			},
		},
		{
			...
			cfg: tun.TUNConfiguration{
				Device: tun.TUNDeviceConfig{Name: "igara0", MTU: 1280},
			},
		},
		{
			name: "blank cidr",
		...
		},
		{
			...
			cfg: tun.TUNConfiguration{
				Device:    tun.TUNDeviceConfig{Name: "igara0", MTU: 1280},
				Addresses: []tun.TUNAddress{{CIDR: "not-a-cidr"}},
			},
		},
	}
...
func TestTUNAddressValidateTableDriven(t *testing.T) {
	tests := []struct {
		...
		{
			name: "trim ipv4 cidr",
			addr: tun.TUNAddress{CIDR: " 172.16.0.2/32\t"},
		},
		{
			name: "trim ipv6 cidr",
			addr: tun.TUNAddress{CIDR: "\n2606:4700:110:8d36::2/128 "},
		},
		{
			name:    "reject malformed cidr",
			addr:    tun.TUNAddress{CIDR: "172.16.0.2"},
			wantErr: true,
		},
	}
...
func TestTUNConfigManagerApplyAndReset(t *testing.T) {
	...
	mgr, err := tun.NewTUNConfigManager(cfg)
	if err != nil {
		t.Fatalf("expected manager creation success, got %v", err)
	}

	dev, err := tun.NewFakeTUNDevice(cfg.Device)
	if err != nil {
		t.Fatalf("expected fake tun device, got %v", err)
	}
	...
	if _, err := tun.NewTUNConfigManager(tun.TUNConfiguration{}); err == nil {
