...
func TestConnectionEvent(t *testing.T) {
	event := kernel.ConnectionEvent{
		State:   kernel.ConnStateConnecting,
		Reason:  "reconnect",
		Attempt: 2,
	}
	if event.State != kernel.ConnStateConnecting {
		t.Fatalf("expected state %q, got %q", kernel.ConnStateConnecting, event.State)
	}
...
func TestKeepaliveManagerEvents(t *testing.T) {
	...
	if events[0].State != kernel.ConnStateReady {
